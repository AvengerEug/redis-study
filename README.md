...
* 3主6从结构搭建步骤(为什么选择3主6从呢？因为想测试下`--cluster-replicas`参数的含义，此参数表示的是每个主节点携带的从节点个数)
  ...
  6. 执行集群命令
     ...
     ==> 按照一定的顺序，比如设置了--cluster-replicas的参数为2，则表示每个主节点有2个从节点，同时redis会计算命令中的节点个数，因为redis-cluster至少是需要3个主节点的。所以会将前面三个ip+port的组合当成主节点，然后去计算 3 * 2 = 6, 然后去验证后面是否有6个从节点，如果无则启动集群失败。
  7. 验证
     1. 登录
